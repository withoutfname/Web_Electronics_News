...
const reviewApp = Vue.createApp({
    data() {
        return {
            ...
            loading: false,
            hasNext: true,
            isAuthenticated: isAuthenticated, // Передано из шаблона
        };
    },
    methods: {
        async fetchReviews() {
        ...
        <div v-for="review in reviews" :key="review.id" class="news-card">
            ...
            <p class="review-meta">
                <span v-if="isAuthenticated">
                    <a :href="'/community/' + review.author__id">Автор: {{ review.author__nickname }}</a>
                </span>
                <span v-else>
                    Автор: {{ review.author__nickname }}
                </span>
                | Дата: {{ formatDateTime(review.created_at) }}
            </p>
